...
def truth_inference(do_neyman=False,
    ...
    if do_neyman:
        ...
        for t in range(settings.n_thetas):
            ...
            np.save(neyman_dir + '/' + neyman_filename + '_llr_null_' + str(t) + '_truth_' + filename_addition + '.npy', llr_neyman_null)

            # Null evaluated at alternative
            llr_neyman_nullatalternative = -2. * np.sum(np.log(r_neyman_null[0]), axis=1)
            np.save(neyman_dir + '/' + neyman_filename + '_llr_nullatalternate_' + '_truth_' + str(
