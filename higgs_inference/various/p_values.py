...
def calculate_median_p_value(llr_distribution, llr_observed):
    ...
    q_cut_index = int(settings.confidence_limit * len(llr_distribution)) - 1
...
def subtract_mle(filename, filename_suffix, folder, neyman2_mode=False):
    ...
    for t in range(n_thetas):
        try:
            entry = np.load(neyman_dir + '/' + neyman_filename + '_llr_distribution_' + filename + '_' + str(
                t) + filename_suffix + '.npy')
            assert entry.shape == (settings.n_thetas, n_neyman_distribution_experiments)
            ...
            logging.warning("File %s has wrong shape %s",
                            neyman_dir + '/' + neyman_filename + '_llr_distribution_' + filename + '_' + str(
        ...
        try:
            llr_observeds.append(
                np.load(neyman_dir + '/' + neyman_filename + '_llr_observed_' + filename + '_' + str(
                    t) + filename_suffix + '.npy'))
            files_found += 1
    ...
    for t_true in range(llr_distributions.shape[1]):
        for exp in range(llr_distributions.shape[2]):
            llr_compared_to_mle_distributions[t_true, exp] = (llr_distributions[t_true, t_true, exp]
                                                              - llr_distributions[
                                                                  theta_mle_distribution[t_true, exp], t_true, exp])
    ...
    # Save results
    np.save(result_dir + '/' + neyman_filename + '_llr_vs_mle_distributions_' + filename + filename_suffix + '.npy',
            llr_compared_to_mle_distributions)
    np.save(result_dir + '/' + neyman_filename + '_llr_vs_mle_observeds_' + filename + filename_suffix + '.npy',
            llr_compared_to_mle_observeds)


def calculate_CL(filename, filename_suffix, folder, neyman2_mode=False):
    """ Steers the calculation of all p-values for a given filename and folder. """

    # Preprocessing
    try:
        subtract_mle(filename, filename_suffix, folder, neyman2_mode)
    except ValueError:
    ...
    # Load LLR
    llr_compared_to_mle_distributions = np.load(
        result_dir + '/' + neyman_filename + '_llr_vs_mle_distributions_' + filename + filename_suffix + '.npy')
    llr_compared_to_mle_observeds = np.load(
        result_dir + '/' + neyman_filename + '_llr_vs_mle_observeds_' + filename + filename_suffix + '.npy')
    ...
    np.save(result_dir + '/' + neyman_filename + '_qmedian_' + filename + filename_suffix + '.npy', q_median_values_mle)


def calculate_all_CL(options=''):
    ...
    calculate_CL('truth', '', 'truth', neyman2_mode)
    calculate_CL('histo', '_2d_new', 'histo', neyman2_mode)
    calculate_CL('scoreregression_score', '', 'score_regression', neyman2_mode)
    calculate_CL('carl', '_shallow_new', 'parameterized', neyman2_mode)
    calculate_CL('combined', '_deep_new', 'parameterized', neyman2_mode)
    calculate_CL('regression', '_new', 'parameterized', neyman2_mode)
    calculate_CL('combinedregression', '_deep_new', 'parameterized', neyman2_mode)
