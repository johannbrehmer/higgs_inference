...
\begin{table}
  ...
  \begin{tabular}{ll rr rr r}
    \toprule
    Algorithm & Setup & \multicolumn{2}{c}{RMS error on $\log  r(\boldx; \boldtheta_0, \boldtheta_1)$}
    & RMS error on $E[\log r(\boldx; \boldtheta, \boldtheta_1) | \boldx \sim \boldtheta_{\text{SM}} ]$ \\
    \cmidrule{3-4}
    && All & Trimmed \\
    \midrule
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   \midrule
   carl (param., raw) & Baseline (2) & 0.21 & 0.07 & 0.8\\
    & Baseline, large batch (2) & \emph{0.20} & 0.06 & \emph{0.5}\\
    & Baseline, LR decay (2) & 0.31 & 0.16 & 5.4\\
    & Baseline, LR decay, large batch size (2) & 0.27 & 0.11 & 6.4\\
    & Baseline, LR decay, small batch size (2) & 0.27 & 0.11 & 6.4\\
    & Baseline, small LR, large batch size (2) & 0.29 & 0.07 & 1.8\\
    & Baseline, small constant LR, large batch size (2) & 0.21 & \emph{0.06} & 1.3\\
    & Baseline, large LR (2) & 0.53 & 0.27 & 6.3\\
    & Baseline, large LR, large batch size (2) & 0.35 & 0.13 & 1.9\\
    & Baseline, large constant LR, large batch size (2) & 0.58 & 0.41 & 27.7\\
    & Random $\boldtheta$ (2) & 0.20 & 0.07 & 0.6\\
   \midrule
   carl (param., cal.) & Baseline (2) & 0.21 & 0.07 & 0.7\\
    & Baseline, large batch (2) & \emph{0.20} & 0.06 & 0.6\\
    & Baseline, LR decay (2) & 0.28 & 0.10 & \\
    & Baseline, LR decay, large batch size (2) & 0.27 & 0.09 & 1.0\\
    & Baseline, LR decay, small batch size (2) & 0.27 & 0.09 & 1.0\\
    & Baseline, small LR, large batch size (2) & 0.26 & 0.06 & 1.0\\
    & Baseline, small constant LR, large batch size (2) & 0.20 & \emph{0.05} & 0.7\\
    & Baseline, large LR (2) & 0.50 & 0.22 & \\
    & Baseline, large LR, large batch size (2) & 0.35 & 0.12 & 1.7\\
    & Baseline, large constant LR, large batch size (2) & 0.45 & 0.19 & 4.3\\
    & Random $\boldtheta$ (2) & 0.21 & 0.07 & \emph{0.6}\\
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \bottomrule
  \end{tabular}
  \caption{Comparison of different inference strategies, part 1.
    ...
    neural networks.  We show two types metrics: the (trimmed) root mean squared
...
\begin{table}
  ...
  \begin{tabular}{ll rr rr r}
    ...
    \midrule
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   carl + score (param., raw) & Baseline (2) & \emph{0.14} & \emph{0.03} & 0.6\\
    & Random $\boldtheta$ (2) & 0.14 & 0.03 & \emph{0.4}\\
   \midrule
   carl + score (param., cal.) & Baseline (2) & 0.12 & \emph{0.03} & \emph{0.4}\\
    & Random $\boldtheta$ (2) & \emph{0.12} & 0.03 & 0.4\\
   \midrule
...
\begin{table}
  ...
  \begin{tabular}{ll rr rr r}
    ...
    \midrule
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   Ratio regression (param., raw) & Baseline (2) & \emph{0.12} & \emph{0.07} & 8.2\\
    & Baseline, large batch (2) & 0.15 & 0.10 & \emph{0.8}\\
    & Baseline, small batch (2) & 0.19 & 0.12 & 2.9\\
    & Baseline, LR decay (2) & 0.20 & 0.13 & 2.3\\
    & Baseline, LR decay, large batch size (2) & 0.22 & 0.16 & 4.7\\
    & Baseline, LR decay, small batch size (2) & 0.22 & 0.16 & 4.7\\
    & Baseline, small LR (2) & 0.20 & 0.14 & 7.3\\
    & Baseline, small LR, large batch size (2) & 0.26 & 0.17 & 1.7\\
    & Baseline, small constant LR (2) & 0.20 & 0.13 & 4.6\\
    & Baseline, small constant LR, large batch size (2) & 0.20 & 0.14 & 3.2\\
    & Baseline, large LR (2) & 0.33 & 0.20 & 2.4\\
    & Baseline, large LR, large batch size (2) & 0.35 & 0.24 & 3.3\\
    & Baseline, large LR, small batch size (2) & 0.20 & 0.12 & 4.2\\
    & Baseline, large constant LR (2) & 0.19 & 0.12 & 3.2\\
    & Baseline, large constant LR, large batch size (2) & 0.21 & 0.15 & 4.6\\
    & Random $\boldtheta$ (2) & 0.22 & 0.15 & 5.2\\
   \midrule
   Ratio regr.\ + score (param., raw) & Baseline (2) & 0.16 & 0.08 & 2.1\\
   Ratio regr.\ + score (param., raw) & Baseline, $\alpha = 30$ (2) & 0.09 & \emph{0.03} & \emph{0.3}\\
   Ratio regr.\ + score (param., raw) & Baseline, $\alpha = 300$ (2) & \emph{0.09} & 0.03 & 1.0\\
   Ratio regr.\ + score (param., raw) & Baseline, $\alpha = 1000$ (2) & 0.17 & 0.09 & 4.5\\
    & Random $\boldtheta$ (2) & 0.17 & 0.11 & 5.5\\
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \bottomrule
  \end{tabular}
...
\begin{table}
  ...
  \begin{tabular}{ll rr}
    \toprule
    Algorithm & Setup & \multicolumn{2}{c}{RMS error on $\left[ \boldt (\boldx; \boldtheta_0, \boldtheta_1) \right]$} \\
    \cmidrule{3-4} 
    && All & Trimmed \\
    \midrule
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
